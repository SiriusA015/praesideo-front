...
const Sections = (props: SectionsProps) => {
  ...
  const handleClickOnAddNewSection = () => {
    ...
    let newValues = props.formConfig?.fields.reduce((obj, { name }) => ({ ...obj, [name]: null }), {});
      ...
      {sections.map((element, index) => (
        <div className={styles.section} key={`section${keys[index]}`}>
          <div className={`${styles.header}`}>
            {props.label && typeof props.label == "string" && props.type =="sections-facility"? (
              <Grid item xs={6} className={styles.sectionsFacility}>
                <TextField
                  fullWidth
                  label="Facility name"
                  variant="filled"
                  color="secondary"
                  value={`${props.label} ${index + 1}`}
                  disabled
                />
              </Grid>):
            ...
            {props.label && typeof props.label == "object" ? (
              <div className={`${styles.name} ${styles.flex}`}>
                {props.label?.map((element, _) => renderLabel(element, index))}
              </div>
            ) : (
              <></>
            )}
